...
import com.jeecg.p3.weixin.util.*;
...
public class WechatServiceImpl implements WechatService {
        ...
        try {
            ...
            //【微信触发类型】事件推送
                ...
                // 扫描二维码
                    ...
                    try {
                        respMessage = doScanQrEventResponse(weixinMessageDTO, request);
                    } catch (Exception e) {
                        e.printStackTrace();
    ...
    /**
     * 【微信触发类型】事件推送-订阅
     * 此处若是扫码关注的公众号，则请求中会带着二维码中的参数，关注模板则需要带团队名，否则不带
     *
    ...
    private String doSubscribeEventResponse(WeixinMessageDTO weixinMessageDTO, HttpServletRequest request) {
        ...
        String templateId = FollowUpConstants.NOT_SCAN_SUB_TEMPLATE_ID;

        String parameter = "";
        if (StrUtil.isNotEmpty(weixinMessageDTO.getKey())) {
            if (weixinMessageDTO.getKey().contains(FollowUpConstants.QR_SCENE)) {
                parameter = weixinMessageDTO.getKey().substring(8);
            } else {
                parameter = weixinMessageDTO.getKey();
            }

            templateId = FollowUpConstants.SCAN_SUB_TEMPLATE_ID;
        }
        // 处理关注事件
        R result = followupService.dealSubEvent(weixinMessageDTO.getFromUserName(), parameter);
        // 关注模板中的团队名称变量
        String teamName = result.get("teamName").toString();
        weixinMessageDTO.setContent(teamName);
        weixinMessageDTO.setTemplateId(templateId);
        return getRespMessage(weixinMessageDTO, "text", request);
    }

    /**
     * 微信触发类型】事件推送-扫码
     * 此处处理微信扫码进入公众号事件，只处理带参数事件，不带参不处理
     * @param weixinMessageDTO
     * @param request
     * @return
     */
    private String doScanQrEventResponse(WeixinMessageDTO weixinMessageDTO, HttpServletRequest request) {
        String respMessage = "";
        // 判断是否为我们自己的带参数的二维码，若不是不处理
        if (StrUtil.isNotEmpty(weixinMessageDTO.getKey())) {
            String parameter = "";
            if (weixinMessageDTO.getKey().contains(FollowUpConstants.QR_SCENE)) {
                parameter = weixinMessageDTO.getKey().substring(FollowUpConstants.QR_SCENE.length());
            } else {
                parameter = weixinMessageDTO.getKey();
            }
            // 处理扫码事件
            ...
            String teamName = result.get("teamName").toString();
            weixinMessageDTO.setContent(teamName);
            weixinMessageDTO.setTemplateId(FollowUpConstants.SCAN_SUB_TEMPLATE_ID);
            respMessage = getRespMessage(weixinMessageDTO, "text", request);
        }
        return respMessage;
    }
    ...
    private String doUnsubscribeEventResponse(WeixinMessageDTO weixinMessageDTO, HttpServletRequest request) {
        ...
        String openid = weixinGzuser.getOpenid();

        Boolean flag = followupService.delBindUnSubscribe(openid);
    ...
    private String getTextRespMessage(WeixinMessageDTO weixinMessageDTO) {
        ...
        if (textTemplate != null) {
            ...
            // TEAM-NAME为固定模板中的参数
            if (content.contains("TEAM-NAME")){
                content = content.replace("TEAM-NAME", weixinMessageDTO.getContent());
            }
        ...
        if (weixinNewsitems != null && weixinNewsitems.size() > 0) {
            ...
            for (WeixinNewsitem weixinNewsitem : weixinNewsitems) {
                ...
                if ("news".equals(weixinNewsitem.getNewType())) {
                    ...
                    if(weixinMessageDTO.getKey().contains("qrscene_")){
                        String substring = weixinMessageDTO.getKey().substring(8);
//                        url+="&parameter="+substring;
                        url+=url.replace("str",substring);
                    }else{
                        url+=url.replace("str",weixinMessageDTO.getKey());
//                        url+="&parameter="+weixinMessageDTO.getKey();
                    }
                }
                log.info("===================url"+url+"=================");
                article.setUrl(url);
    ...
    public void saveGzUserInfoByOpenId(String openId, String jwid) {
        ...
        if (weixinGzuser == null) {
            ...
            this.weixinGzuserService.doEdit(gzUser);
        }
    }
